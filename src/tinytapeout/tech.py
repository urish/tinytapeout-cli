...
ASIC_TECHS: list[TechName] = ["sky130A", "ihp-sg13g2", "gf180mcuD"]


def load_tile_sizes(pdk: TechName, tt_tools_dir: str | Path) -> dict[str, str]:
    """Load tile_sizes.yaml for a given PDK from tt-support-tools."""
    import yaml

    path = Path(tt_tools_dir) / "tech" / pdk / "tile_sizes.yaml"
    with open(path) as f:
        return yaml.safe_load(f)


def load_cells(pdk: TechName, tt_tools_dir: str | Path) -> dict:
    """Load cells.json for a given PDK from tt-support-tools."""
    path = Path(tt_tools_dir) / "tech" / pdk / "cells.json"
    with open(path) as f:
        return json.load(f)
...
class Tech(Protocol):
    ...
    librelane_config: dict[
        str, bool | int | float | str | list[str] | dict[str, list[str]]
    ]
    label_layers: list[tuple[int, int]]
    buried_layers: list[tuple[int, int]]
    scramble_cells: None | str
    ...
    def load_cell_definitions(
        self, tt_tools_dir: str | Path
    ) -> dict[str, CellDefinition]:
        raise NotImplementedError()
...
class Sky130Tech(Tech):
    ...
    def load_cell_definitions(
        ...
        cells = load_cells("sky130A", tt_tools_dir)
...
class IHPTech(Tech):
    ...
    def load_cell_definitions(
        ...
        cells = load_cells("ihp-sg13g2", tt_tools_dir)
...
class GF180MCUDTech(Tech):
    ...
    def load_cell_definitions(
        ...
        cells = load_cells("gf180mcuD", tt_tools_dir)
        for name, cell in cells.items():
